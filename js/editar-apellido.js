/*PARA MI NO SIRVE ESTO CHEQUEAR!!!!!------------------------------------------------------
const countries = [
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        mostrarMensaje('Error al obtener los datos del usuario', 'error');
...
editarApellido.addEventListener("click", async () => {
    try {
        ...
        const response = await fetch(`https://malaria-xi.vercel.app/user/editApellido/${id}`, {
            method: "POST",
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
        ...
        mostrarMensaje('Error al editar los datos del usuario', 'error');
        console.error(err);
    }
});
